...
public partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(AppConfig config)
    {
        ...
        StartWithWindowsCheckBox.IsChecked = _draftConfig.StartWithWindows;
    ...
    private void SaveAndRebuildButton_Click(object sender, RoutedEventArgs e)
    {
    ...
    private void SaveAndClose(bool rebuildWordBank)
    {
        ...
        {
            MessageBox.Show("切换间隔请输入数字。", "设置", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }

        if (intervalSeconds < 3 || intervalSeconds > 3600)
        {
            MessageBox.Show("切换间隔请设置在 3 到 3600 秒之间。", "设置", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        _draftConfig.StartWithWindows = StartWithWindowsCheckBox.IsChecked == true;
    ...
    {
        OpacityValueTextBlock.Text = $"{Math.Round(OpacitySlider.Value):0}%";
    }
}
